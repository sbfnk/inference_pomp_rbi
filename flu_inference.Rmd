...
This tutorial is based on the tutorial on [Iterated filtering: principles and practice](https://kingaa.github.io/sbied/mif/mif.html#building-up-a-picture-of-the-likelihood-surface) by Edward Ionides and Aaron A. King.

It was produced with **R** version `r getRversion()`, **pomp2** version `r packageVersion("pomp2")` and **rbi** version `r packageVersion("rbi")` on an Intel Xeon E5-2603 v3 1.6GHz (6-core) CPU and Nvidia Tesla P100 16GB NVLink GPU.
...
The data are provided with the **pomp2** package in the `bsflu` object.
To find out how to install the **pomp2** package, read at the [pomp2 installation instructions](https://kingaa.github.io/pomp/install.html).
...
# Inference using iterated filtering with pomp2

We first conduct inference using the IF2 iterated filtering algorithm in **pomp2**.
For more information on **pomp2**, consult the [pomp website](https://kingaa.github.io/pomp/), which contains plenty of documentation and examples.
...
We first load the necessary libraries and formulate the model

```{r rbi_init}
library(rbi)
library(rbi.helpers)
library(stringi) ## for reading the model from a string
library(pomp2) ## for the bsflu data set
library(tidyverse)
...
As with the **pomp2** example, we start with testing the model syntax and running some simulations.
...
This, once again, does a series of test runs and adapts the proposal distribution after every run usingn the empirical covariance of accepted samples.
...
Generating the samples took `r round(bi_read(posterior)$clock/1e+6/60)` minutes.
...
Using the GPU, generating 10,000 samples only took `r round(bi_read(posterior)$clock/1000/1000/60)` minutes.

## pMCMC in pomp2

The *pomp2* package also implements pMCMC.
...
Generating the 10,000 samples with **pomp2** took `r as.integer(round(difftime(end, start, units='mins')))` minutes altogether.
